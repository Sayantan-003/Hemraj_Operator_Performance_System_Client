...
const DEOSectionReport = () => {
  ...
  const [parameters, setParameters] = useState([
    ...
    { name: "Unit Consumed", value: "1200 mT" },
  ]);
  ...
  useEffect(() => {
    const fetchFilteredData = async () => {
      ...
      try {
        ...
        const params = [];
        if (filter.operator)
          params.push(`operator=${encodeURIComponent(filter.operator)}`);
        if (filter.hours)
          params.push(`hours=${encodeURIComponent(filter.hours)}`);
        if (filter.shift)
          params.push(`shift=${encodeURIComponent(filter.shift)}`);
